...
int linux_read(Network* n, unsigned char* buffer, int len, int timeout_ms)
{
	...
	while (bytes < len)
	{
		...
		if (rc == -1)
		{
			...
			{
				log_printf(LOG_ERROR, "[MqttSocket] failed\n");
				bytes = -1;
				break;
			}
		}
		else if(rc ==0)
		{
			...
			log_printf(LOG_ERROR, "[MqttSocket]sever close the socket\n");
			bytes = -1;
			break;
		}
		else
...
#ifdef EV_WRAP
int linux_write_ev(Network* n, unsigned char* buffer, int len)
{
...
int ConnectNetwork(Network* n, char* addr, int port)
{
	...
	if (rc == 0)
	{
		...
		if (n->my_socket != -1)
		{
			setnonblock(n->my_socket);	// Set it non-blocking  //add by yanly
			rc = connect(n->my_socket, (struct sockaddr*)&address, sizeof(address)); //放在这里有没有问题？待测试
		}
	}
	return rc;
}
